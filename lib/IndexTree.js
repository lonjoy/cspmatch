 ...
 * var ret = index.matchOne('句子');              // 匹配，失败返回false
 * var ret = index.match('句子');   	            // 匹配一段，返回数组
 */
...
var debug;
if (process.env.QUICKWEB_DEBUG && /index/.test(process.env.QUICKWEB_DEBUG))
  debug = function(x) { console.error('Index: %s', x); };
else
  debug = function() { };
...
IndexTree.prototype.match = function (text) {
  var words = this.segment1.doSegment(text);
  var rets = [];
  var s = 0;
  var i = 0;
  while (i < words.length) {
    var w = words[i].w;
    if (w in SPLIT_D_W) {
      var line = words.slice(s, i);
      s = i + 1;
      var ret = this.matchOne(this.segment1.toString(line));
      rets.push(ret);
    }
    i++;
  }
 ...
 * @param {string} text 文本
 */
IndexTree.prototype.matchOne = function (text) {
